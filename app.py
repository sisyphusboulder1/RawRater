...
if st.session_state.mode is not None:
    ...
    # Handle uploaded files differently based on mode
    if uploaded_files is not None:
        max_files = 1 if st.session_state.mode == "individual" else 3
        # Convert single file to list for consistent processing
        files_to_process = [uploaded_files] if st.session_state.mode == "individual" else uploaded_files
        if len(files_to_process) <= max_files:
            # Process and crop images
            st.session_state.cropped_images = []
            st.session_state.image_data = []
            for i, uploaded_file in enumerate(files_to_process, 1):
            ...
            if st.session_state.cropped_images:
                ...
                prompts = {
                    "Rate": (
                        f"You’re a balanced critic. {'This image shows one man' if st.session_state.mode == 'individual' else f'These {len(files_to_process)} cropped images show men (solo or groups)'}. "
                    ...
                    "Unhinged Rating": (
                        f"You’re a wild, unhinged critic. {'This image shows one man' if st.session_state.mode == 'individual' else f'These {len(files_to_process)} cropped images show men (solo or groups)'}. "
                    ...
                    "Feedback": (
                        f"You’re a blunt, helpful critic. {'This image shows one man' if st.session_state.mode == 'individual' else f'These {len(files_to_process)} cropped images show men (solo or groups)'}. "
                    ...
                    "Roast Me Dead": (
                        f"You’re a merciless roasting machine. {'This image shows one man' if st.session_state.mode == 'individual' else f'These {len(files_to_process)} cropped images show men (solo or groups)'}. "
            ...
            st.error(f"Chill, bro—max {max_files} image{'s' if st.session_state.mode == 'group' else ''}.")
